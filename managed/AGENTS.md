...
- 复杂任务指需联合推理跨模块契约, 并发或生命周期不变量, 或难以局部验证的取舍. 仅复杂任务自动派发 planner / reviewer, 用户明确要求时例外; 文件数, 步骤数和阶段完成不构成依据.
- `/root` 安排复杂任务的关键规划和可独立验收阶段的审查; 已有适用方案不重复规划. 普通任务由执行者规划, 自查和验证.
- 规划按问题, 审查按阶段和比较基准划分主题. 同题复用 owner, 必要交接时例外; 新主题独立判断. 无相关变更或新证据不重复规划或审查.

### `/root`

- 派发须有独立产出, 综合考虑上下文节省与交接, 等待, 重复读取成本; 收益不替代派发条件.
- `/root` 统一决定重分派和交接. 子代理沿 parent 链上报未解问题, 已验证事实, 排除方案和最新验收证据.
- 已确认未修复的 bug 交由实现 owner 处理. 缺输入或证据先补, 不可得则上报; 路线失效或职责不符由 `/root` 重分派, 不扩大授权.
- 有实质进展则继续, 不机械计数; 续作须说明新假设或证据来源及验证方式, 不重复已排除路线.
- 同题按契约问题及相关触发条件或因果链识别, 独立根因另算. 换人, 换窗或跨角色交接均保留事实和尝试证据, 不通过换人重复无效尝试.
- 同题交接前结束或中断原 owner 的相关工作, 只读角色亦如此. 实现交接另须确认原 owner, 后代和后台命令均已停止相关写入且不会恢复. 移交修改, 约束, 验收边界, 验证结果和未决问题. 交接仅用于原 owner 无法继续或职责变更.
