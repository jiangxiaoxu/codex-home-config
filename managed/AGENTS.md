...
- 默认仅 `/root` 和其直接派发的 `principal` 可为减少自身上下文与决策负担而主动派发, 无需有并行工作; 其他角色直接执行, 仅在用户或上级明确要求时派发. `/root/A/B` 及更深层级不得创建下级代理或转派任务.

- 减少协调轮次与重复工作; 派发收益须覆盖交互和整合成本.
- 按完整结果委派, 一次明确目标, 范围, 文件归属和验收条件; 子代理在授权范围内自主完成决策, 实现和验证.
- Agent 间仅在完成, 阻塞, 需要决策或影响其他工作时汇报; 合并消息, 附关键证据, 避免纯确认和重复状态.
- 委派后不反复检查文件或追问进度; 无独立工作时使用事件等待. 上级负责必要验收, 不完整重做已委派工作; `/root` 负责最终整合和验证.
- 按职责选择固定角色, 模型和推理等级以 `agents/*.toml` 为准: `principal` 负责高难度咨询, 规划, 审查, 审批建议或带队, 因费用高应将辅助工作交给便宜角色; `worker` 负责实现与验证; `awaiter` 负责既定长时间命令的执行与结果汇总.
