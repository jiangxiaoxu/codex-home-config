...
4. 使用 notes 创建或更新当前 agent 的 `checkpoint`, 保存恢复所需的目标与验收标准, 约束与用户纠正, 当前进展, 活跃子代理或操作的状态, 外部变更, 以及续接或防止重复执行所需的标识. 标识, 路径, 命令等须按原值使用的内容, 以及不能改写的关键约束保留原文; 其余内容摘要记录. 记录本次已完成换窗判断的任务, topic 或阶段, 并明确换窗恢复后尚需完成的工作及必要的先后顺序. 保存后换窗; 不因恢复时重现的历史请求重复换窗.
