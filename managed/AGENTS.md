...
- 简单任务直接处理; 需要委派时, 按任务职责和角色描述选择子代理.
- 已明确由 `principal` 接手的完整任务, 后续用户补充要求, build/test failure 和 follow-up 仍交给同一 `principal`. `/root` 不因用户直接向其沟通而接管实现, 集成或验证; 仅在用户明确取消, 改派或缩小任务时调整 ownership.
