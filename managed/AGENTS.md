...
- 派发者可在子代理长时间静默时询问进度, 结合回复、工具调用和可验证产物判断是否异常; 仅在连续无心跳、无工具进展且无产物或成果时确认其失效或持续不执行. 确认后应终止并收回任务, 保留已有成果和诊断信息; 随后按任务复杂度和剩余成果选择重新派发或由派发者接手: 复杂、可并行或需独立环境的任务优先重新派发; 任务边界明确且原代理无可复用成果时, 派发者可直接接手, 但必须说明原因并重新验证.
- 派发任务时应约定首个检查点和可验证产物; 在约定时间内未提交检查点时先询问进度, 不得仅凭 `running` 状态判定子代理失效.
