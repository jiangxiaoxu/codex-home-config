...
- 派发决策顺序如下: 对大批量代码检索或仓库探索任务,优先使用`explorer`; 对明显属于既定专项职责域的任务,优先复用或派发对应`worker`; 当`explorer`和`worker`都不合适时,再选择其他合适的`子代理`.
- 对构建/测试职责域任务,`主线程`默认应优先复用现有的构建/测试专项`worker`; 不存在可复用实例时再创建新的同类专项`worker`. 构建/测试专项`worker`只负责构建,测试,日志收集和结果归纳,不得混用为搜索,产品分析或实现类代理.
- 仅在以下情况之一成立时,`主线程`可直接执行而不派发`子代理`: 任务极短小且预期只需极少上下文; 派发开销明显高于收益; 单次快速检索; 单条轻量校验命令; 工具限制明确要求必须由`主线程`执行; 或当前下一步被单个轻量操作直接阻塞,由`主线程`立即完成更快.
- 专项`worker`应视为长生命周期,可复用的工作实例. 已有任务类型一致且既有结论仍可直接复用的实例时应优先复用,不因`主线程`"也能做"而跳过派发,默认不因单次任务完成立即关闭.
- 仅在以下情况之一成立时,才新建,重建或回收专项`worker`: 代理类型或职责不匹配; 实例已关闭或失效; 当前 repo,分支,工作目录或关键任务前提已变化,导致既有结论不再适用; 当前任务的工作类型已明显变化; 用户明确要求回收或重开; 或需要并行分工且职责边界清晰.
