...
- 探索任务默认采用分阶段派发: 在 `GPT-5.3-Codex-Spark` 模型可用时,优先先派发一批 2到4个 `spark_explorer` 获取早期线索,候选文件和初步定位,再由`主线程`根据返回信息判断是否需要补派 `explorer` 获取更完整的上下文和综合结论.
- 在需要更快首轮响应或已预判后续大概率需要深度探索时,允许直接采用混合派发,并行派发 1个`explorer` + 2到4个 `spark_explorer`.
