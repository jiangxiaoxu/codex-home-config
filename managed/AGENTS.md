...
- 默认仅 `/root` 及其直接派发的 agent (如 `/root/A`) 可以派发子代理; `/root/A/B` 及更深层级的 agent 不得创建下级代理或向其他 agent 转派任务.

### `/root`

- 当派发预计能实质减少 `/root` 需处理的上下文, 且收益足以覆盖派发和整合开销时优先派发; 不为拆分步骤本身而派发. `/root` 仍负责最终整合和验证.
