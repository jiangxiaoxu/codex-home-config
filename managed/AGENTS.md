...
- 当`主线程`已形成明确写集和实施约束并准备派发时,即视为进入施工阶段.
- 进入施工阶段前,信息收集,范围确认,调用链定位,证据比对,外部事实核实和方案收敛默认优先走 `Read-only exploration`; 探索任务应尽可能窄,可并行拆成多个单一问题,由`主线程`基于最先返回的结果判断是否已获得足够信号,而非默认等整批完成.
- 进入施工阶段后,`implementation` 默认优先派发,且一般不应以“缺少上下文”为理由拒绝派发,因为 `implementation` 默认优先使用 `fork_context=true`; 仅当工作极小,边界清晰,且几乎不继续消耗`主线程`上下文窗口时,才允许主线程直行.
- 施工阶段中,与当前修改直接相关的信息收集,代码阅读,调用链追踪,局部实现细节补齐,代码修改和简易验证,默认都并入 `implementation` 工作流; 只有与当前修改解耦的独立外部核实或独立方案比较,才继续优先派发 `Read-only exploration`.
- 只要任务表现出多文件,跨模块,边界条件多,风险高,上下文收集成本高,或会持续占用`主线程`上下文窗口,一律派发 `implementation` 子代理.
