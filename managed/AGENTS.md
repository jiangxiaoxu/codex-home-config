...
- 原 owner 保留整合和验收责任; 并行写入的 ownership 不得重叠.超出 owner 边界的工作按新 topic 派发.
...
- `planner` / `reviewer` 的自动派发统一由 `/root` 负责, 适用范围仅限复杂任务的关键规划和可独立验收阶段的审查; 用户明确要求时按其指定范围和责任执行.
...
- 需要按本文件的`澄清`规则向用户确认的产品行为或风险取舍, 仍由 `/root` 负责, 不由子代理代为决定.
