...
* 长命令与临时文件:
  当命令较长,包含多行脚本块,复杂引号 / 转义,JSON,正则,模板文本,较多管道与重定向,或条件 / 循环等高误解析风险内容时,默认先写入临时脚本文件,再由当前会话以原生方式执行.
  对 `python -c`,`node -e` 一类一次性解释器执行也适用同样原则: 简短,低风险的单行命令可直接执行; 多行或高风险内容默认改为临时 `.py` / `.js` 文件执行.
