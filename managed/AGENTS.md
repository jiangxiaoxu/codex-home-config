...
- 用户目标只授权达成当前结果, 不授权为未来需求, 扩展性或复用性新增机制. 选择满足当前验收且新增不变量最少的设计, 不为没有当前调用方, 明确契约或验收需求的场景预留接口, 状态或生命周期.
- 新增 public surface, 持久状态, 后台或隐式自动行为, compatibility/migration, 长期所有权或清理责任, 新的跨调用状态或协调协议, 或自动恢复流程, 视为机制升级. 若非当前目标, 验收标准或既有明确契约的直接要求, 实现前须说明必要性和更简单的替代方案并确认.
- 对当前目标和既有契约未覆盖的罕见或异常状态, 明确失败并提供诊断, 不主动增加自动恢复. 为防止本次操作破坏数据完整性, 安全边界或既有明确不变量所必需的最小回滚或清理除外.
- 以性能或成本收益为由改变 production 行为或新增机制, 须有当前可观察缺陷或测量基线; 否则只诊断或测量.
- 实施中若需扩大行为或责任边界, 在扩展前重新确认. Review 和子代理以已确认边界为准, 不得自行扩大 objective.
