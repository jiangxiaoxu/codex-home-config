...
4. 使用 notes 创建或更新当前 agent 的 `checkpoint`, 保存恢复所需的目标与验收标准, 约束与用户纠正, 当前进展, 活跃子代理或操作的状态, 外部变更, 以及续接或防止重复执行所需的标识; 需要精确保留的内容使用原文. 记录本次已完成换窗判断的任务, topic 或阶段, 并明确换窗恢复后尚需完成的工作及必要的先后顺序. 保存后换窗; 恢复后按记录继续推进, 不因同一历史请求仍然可见而重复换窗.
