...
- 当前 `AGENTS.md` 或其他 active user-scoped instruction file 中,对 `spawn_agent`,`子代理`,委托或并行代理工作的明确授权,预授权或指示,即视为用户已明确要求`主线程`自主调用 `spawn_agent`.

### 总体原则

- `主线程`可以自主决定是否调用`spawn_agent`.
- `主线程`负责`子代理`协作的前置检查,任务拆分,参数整理,等待结果,汇总结论和对外说明.
- `主线程`应优先承担编排,整合,决策和对外说明职责; 对不会阻塞当前关键路径的探索,检索,验证,构建,测试和日志收集工作,默认优先委托给`子代理`.
- `主线程`可根据任务结构,并行收益,任务类型是否一致,预期上下文开销,可复用的`子代理`实例情况,以及可用`子代理`的职责说明,能力特点和适用场景,决定是否派发并自主选择更合适的`子代理`角色; 在合适的情况下,通常优先考虑派发.
- `explorer`用于代码检索和仓库探索,`worker`用于执行类专项任务; 当二者都不合适时,`主线程`可自行选择其他合适的`子代理`类型.

### 派发与复用

- 对高上下文开销任务,`主线程`默认应优先派发`子代理`. 高上下文开销任务包括但不限于: 需要跨多个目录或模块进行搜索/定位/追踪,需要在仓库内做大范围探索,需要梳理较长调用链,需要汇总大量候选文件或证据,以及预计会产生较长中间结果的构建/测试任务. 其中代码检索和仓库探索类任务默认更应优先考虑`explorer`,因为其实际使用小模型,成本低且适合高频派发.
- 对彼此独立且可以并行推进的问题,`主线程`默认应优先并行派发多个职责清晰的`子代理`,而不是由`主线程`串行完成.
- 派发决策顺序如下: 对代码搜索,定位,调用链梳理,候选文件收敛,仓库结构摸底等检索和探索任务,默认优先使用`explorer`,不以"任务是否足够大"作为前提; 对明显属于既定专项职责域的任务,优先复用或派发对应`worker`; 当`explorer`和`worker`都不合适时,再选择其他合适的`子代理`.
- 对构建/测试职责域任务,`主线程`默认应优先复用现有的构建/测试专项`worker`; 不存在可复用实例时再创建新的同类专项`worker`. 构建/测试专项`worker`只负责构建,测试,日志收集和结果归纳,不得混用为搜索,产品分析或实现类代理.
- 仅在以下情况之一成立时,`主线程`可直接执行而不派发`子代理`: 任务极短小且预期只需极少上下文; 派发开销明显高于收益; 单条轻量校验命令; 工具限制明确要求必须由`主线程`执行; 或当前下一步被单个轻量操作直接阻塞,由`主线程`立即完成更快. 对轻量检索任务,除非满足前述例外,否则仍应优先派发`explorer`,而不是由`主线程`自行处理.
- 已将某个目标问题派发给`explorer`后,`主线程`应等待该`explorer`返回,不要同时由`主线程`自身对同一目标问题并行执行检索,定位或追踪. 如需额外检索信息,应继续派发新的`explorer`,而不是改由`主线程`自行并行检查. 这样可以减少无谓的主线程上下文窗口消耗.
- 专项`worker`应视为长生命周期,可复用的工作实例. 已有任务类型一致且既有结论仍可直接复用的实例时应优先复用,不因`主线程`"也能做"而跳过派发,默认不因单次任务完成立即关闭.
- 满足以下情况之一则需要新建,重建或回收专项`worker`: 代理类型或职责不匹配; 实例已关闭或失效; 当前 repo,分支,工作目录或关键任务前提已变化,导致既有结论不再适用; 当前任务的工作类型已明显变化; 用户明确要求回收或重开; 或需要并行分工且职责边界清晰.
...
- 在已派发的`explorer`返回结果之前,`主线程`应等待结果,不要由`主线程`自身对同一目标问题重复或并行执行检索工作; 如需补充额外线索,可以继续派发新的`explorer`. 同一目标问题是指: 为回答同一个待解决问题,在相近范围内进行相同目的的搜索,定位,追踪或候选收敛.

### spawn_agent 约束

- 所有`子代理`只能由`主线程`创建,调度和回收,且`子代理`线程中不得再次调用`spawn_agent`.
- 调用`spawn_agent`时,必须显式使用`fork_context=false`,以减少无关线程历史注入并保持专项上下文稳定.
- 调用`spawn_agent`后,`主线程`需要说明本次派发的目的,目标子代理角色,所用模型和推理等级.
- 调用`wait_agent`时,构建/测试专项`worker`默认超时为`1800000` ms; 其他类型`子代理`默认超时为`600000` ms. 可根据任务规模,阻塞程度和预期运行时长调整.
