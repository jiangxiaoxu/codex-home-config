...
- `spark_explorer`默认按小批量并发派发; 需要更大范围探索时按批次扩展,避免一次性无上限派发. 该模型不可用时,回退到 `explorer`.
...
- 探索任务默认采用分阶段派发: 在 `GPT-5.3-Codex-Spark` 模型可用时,优先先派发一批 2到3个 `spark_explorer` 获取早期线索,候选文件和初步定位,再由`主线程`根据返回信息判断是否需要补派 `explorer` 获取更完整的上下文和综合结论.
- 在需要更快首轮响应或已预判后续大概率需要深度探索时,允许直接采用混合派发,并行派发 1个`explorer` + 2到3个 `spark_explorer`.
- 对高上下文开销任务和彼此独立的问题,默认优先派发`子代理`,而不是由`主线程`串行处理. 代码搜索,候选文件收敛,仓库摸底等探索任务不以 `"任务是否足够大"` 为前提,默认应优先考虑探索型`子代理`.

### 派发后的主线程探索约束

- 同一目标问题一旦已派发给探索型`子代理`,在探索型`子代理`返回结果之前,`主线程`默认禁止亲自进行常规探索;需要等同一批次的全部探索型`子代理`都返回结果之后再继续判断,整合,并按需追加派发更多探索型`子代理`,直到拿到所需信息为止. 这里的"常规探索"包括但不限于代码搜索,候选文件收敛,调用链追踪,批量文件读取和实现摸底.
