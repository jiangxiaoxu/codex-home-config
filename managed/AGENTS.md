...
- 用户已授权主线程可以自主决定调用`spawn_agent`.
- 主线程负责子代理协作的前置检查,任务拆分,参数整理,等待结果,汇总结论和对外说明.
- 主线程可基于任务结构,并行收益,专项职责匹配和上下文规模,自主派发子代理.
- 调用`spawn_agent`后需要说明派发子代理的目的和目标代理类型以及使用的AI模型和推理等级.
- `explorer` 用于代码检索和仓库探索, `worker` 用于执行类专项任务; 所有子代理只能由主线程创建,调度和回收,且子代理线程中不得再次调用 `spawn_agent`.
- 调用 `spawn_agent`时,必须显式使用 `fork_context=false`,以减少无关线程历史注入并保持专项上下文稳定.
- 派发决策顺序: 对于大批量代码检索或仓库探索任务,主线程应优先使用 `explorer`; 对于明显属于既定专项职责域的任务,主线程应优先复用或派发对应 `worker`; 当 `explorer` 和 `worker` 都不合适时,主线程可自行选择其他合适的子代理.
- 主线程可直接执行的例外: 仅当任务极短小,派发开销大于收益,单次快速检索,单条轻量校验命令,或工具限制明确要求必须由主线程直接执行时,可不派发子代理.
...
- 调用 `wait_agent` 时,构建/测试专项 `worker` 默认超时为 `1800000` ms; 其他类型子代理默认超时为 `600000` ms. 可按任务规模和阻塞程度调整.
