...
- 探索任务默认采用分阶段派发: 在 `GPT-5.3-Codex-Spark` 模型可用时,优先先派发一批 `1-3 spark_explorer` 获取早期线索,候选文件和初步定位,再由`主线程`根据返回信息判断是否需要补派 `explorer` 获取更完整的上下文和综合结论.
- 在需要更快首轮响应或已预判后续大概率需要深度探索时,允许直接采用混合派发,并行派发 `1 explorer + 2-4 spark_explorer`.
- 对高上下文开销任务和彼此独立的问题,默认优先派发`子代理`,而不是由`主线程`串行处理. 代码搜索,候选文件收敛,仓库摸底等探索任务不以 `"任务是否足够大"` 为前提,默认应优先考虑探索型`子代理`.

### 复用、失效与直行例外

- 同一目标问题一旦已派发给探索型`子代理`,`主线程`默认不再亲自进行常规探索,而应基于已返回结果继续判断,整合,并按需追加派发更多探索型`子代理`,直到拿到所需信息为止.
