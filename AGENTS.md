...
* Install-generated managed update fragments follow `managed/config.toml` order. Publish-sync preserves existing managed key order recursively and appends eligible new keys in local order; values and array element order follow local configuration. Top-level and MCP-child allowlists still apply. Unmanaged target top-level entries/table blocks retain their original relative positions.
